...
public sealed class StockGrnAC
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return await StockRecordResponseReader.ReadAsync<StockGrnEnvelopeDTO, StockGrnDocumentDTO>(
            response,
            ["mobjDoc_Stock_GRN", "objDoc_Stock_GRN"],
            (document, lines) => new StockGrnEnvelopeDTO
            {
                Document = document ?? new StockGrnDocumentDTO(),
                DocumentLines = lines
            },
            allowEmptyDocument: string.IsNullOrWhiteSpace(id),
            requestFailureMessage: "GRN API request failed.",
            invalidResponseMessage: "GRN record response was invalid.",
