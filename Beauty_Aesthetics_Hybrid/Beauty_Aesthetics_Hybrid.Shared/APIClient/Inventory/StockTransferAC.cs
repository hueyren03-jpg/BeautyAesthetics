...
public sealed class StockTransferAC
{
    ...
    public async Task<ApiCallResult<StockTransferEnvelopeDTO>> LoadRecordAsync(
        string id,
        CancellationToken cancellationToken = default)
    {
        ...
        using var response = await authService.SendAuthorizedAsync(request, cancellationToken);
        return await StockRecordResponseReader.ReadAsync<StockTransferEnvelopeDTO, StockTransferDocumentDTO>(
            response,
            ["objDoc_StockTransfer", "mobjDoc_StockTransfer"],
            (document, lines) => new StockTransferEnvelopeDTO
            {
                Document = document ?? new StockTransferDocumentDTO(),
                DocumentLines = lines
            },
            allowEmptyDocument: string.IsNullOrWhiteSpace(id),
            requestFailureMessage: "Stock transfer API request failed.",
            invalidResponseMessage: "Stock transfer record response was invalid.",
